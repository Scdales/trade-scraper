...
class HarmonicDetector(object):
    ...
    def detect_bat(self, current_pat: list, predict: bool = False, predict_mode: str = 'direct'):
        ...
        if not predict:
            ...
            pattern_found = np.all(np.array([
                ...
                self.is_eq(ABCD, 1) or self.is_in(
                    ABCD, 0.786, 1.618),  # AB=CD and its alternatives
            ]))
            direction = 1 if D < C else -1
            if pattern_found:
                return [direction, ret_dict]
    ...
    def detect_crab(self, current_pat: list, predict: bool = False,  predict_mode: str = 'direct'):
        ...
        if not predict:
            ...
            pattern_found = np.all(np.array([
                ...
                self.is_eq(ABCD, 1) or self.is_in(
                    ABCD, 0.786, 2.236),  # AB=CD and its alternatives
            ]))
            direction = 1 if D < C else -1
            if pattern_found:
                return [direction, ret_dict]
    ...
    def get_patterns(self, df: pd.DataFrame, window: int, predict: bool = False, plot: bool = False):
        ...
        if plot:
            ...
            mpf.plot(df, type='candle', alines=dict(
                alines=patterns_line, colors=['b', 'r', 'c', 'k', 'g']))
        if predict:
        ...
        if len(patterns_line) > 0:
            if save_fig and file_name is not None:
                ...
                mpf.plot(df,
                         ...
                         hlines=dict(hlines=predict_Ds, colors=[
                                     'g'], linestyle='-.'),
                         savefig=file_path,
                         figsize=(12.8, 9.6),
                         # dpi=400
    ...
    def search_patterns(self, df: pd.DataFrame, predict: bool = False, only_last: bool = False, last_n: int = 0, plot: bool = False, plot_predict: bool = True, return_dt_idx=True, save_fig_name: str = None):
        ...
        if only_last and last_n > 0:
            all_patterns = [pat for pat in all_patterns if pat[0]
                            [-1][2] == len(df) - last_n]

        all_patterns = self.filter_duplicats(all_patterns)
        all_predict_patterns = self.filter_duplicats(
            all_predict_patterns, predict=True)
        if plot:
            if save_fig_name is not None:
                self.plot_patterns(
                    df, all_patterns, all_predict_patterns, save_fig=True, file_name=save_fig_name)
            else:
                self.plot_patterns(
                    df, all_patterns, all_predict_patterns, save_fig=False)
